export class CrudPermissions {
  static readonly IS_ADD_PRODUCT_AVAILABLE: boolean = true;
  static readonly IS_UPDATE_PRODUCT_AVAILABLE: boolean = true;
  static readonly IS_DELETE_PRODUCT_AVAILABLE: boolean = true;

  static readonly IS_ADD_CATEGORY_AVAILABLE: boolean = true;
  static readonly IS_UPDATE_CATEGORY_AVAILABLE: boolean = true;
  static readonly IS_DELETE_CATEGORY_AVAILABLE: boolean = true;
}
